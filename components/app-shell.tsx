              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={isActive ? "relative text-sm font-medium text-white after:absolute after:-bottom-[11px] after:left-1/2 after:h-[2px] after:w-10 after:-translate-x-1/2 after:rounded-full after:bg-[#CEF17B] after:shadow-[0_0_18px_rgba(206,241,123,0.9)]" : "text-sm font-medium text-slate-400 transition hover:text-white"}
                  >
                    {item.label}
                  </Link>
                );
